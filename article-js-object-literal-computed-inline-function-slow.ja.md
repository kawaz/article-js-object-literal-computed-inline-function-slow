...
| 条件 | 結果 | 検証 |
|---|---|---|
| オブジェクトリテラルで生成 | △ 単独では問題ない | 検証1 |
| computed property を使う | △ 単独では問題ない | 検証1 |
| 毎回新しい関数を生成する | △ 単独では問題ない | 検証4 |
| メソッド定義構文を使う | △ 単独では問題ない | 検証6 |
| 値がプリミティブではなく関数 | △ 関数の場合だけ遅い | 検証4 |
...
- **変数経由**（ローカル変数でもモジュールスコープでも）なら、関数定義がリテラル外なので最適化が効く
