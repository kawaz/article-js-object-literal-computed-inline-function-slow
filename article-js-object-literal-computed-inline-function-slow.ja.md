...
対応の優先度としては **変数経由** > **後付け** > **class化** の順で検討するのが良いと思う。

- **変数経由**: おそらく最も簡単かつパフォーマンスも最良。余計な Shape transition が発生しない
- **後付け**: 簡単だが、computed property が多いと transition が増える
- **class化**: 影響範囲が大きいリファクタリングになる

書き換えパターンは機械的なので、ESLint ルールでの検出・自動修正も比較的簡単に実装できそうだ。同様の変換は V8 / JSC 側の最適化パスに組み込むこともできるのではないか。
