...
ただし、これは最初の Shape 生成時だけの話で、同じパターンで2個目以降を作るときは既存のチェーンが再利用される。なのでこれが大きな問題になる事は少ないはず（ハードコードされたコードでは追加順が固定されるため、問題になるのはループで不定なフィールドリストから動的生成する場合くらいだと思う）。なのであまり神経質になる必要はないと思う。
